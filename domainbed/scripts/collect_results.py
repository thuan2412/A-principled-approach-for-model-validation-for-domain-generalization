...
from domainbed.model_selection import NAME
...
def print_results_tables(records, selection_method, latex):
    ...
    if selection_method.name == "training-domain mmd validation set":
        grouped_records = reporting.get_grouped_records(records).map(lambda group:
            { **group, "sweep_acc": selection_method.sweep_mmd_loss(group["records"]) }
        ).filter(lambda g: g["sweep_acc"] is not None)
    elif selection_method.name == "training-domain coral validation set": 
        grouped_records = reporting.get_grouped_records(records).map(lambda group:
            { **group, "sweep_acc": selection_method.sweep_coral_loss(group["records"]) }
        ).filter(lambda g: g["sweep_acc"] is not None)
    else:
        grouped_records = reporting.get_grouped_records(records).map(lambda group:
            { **group, "sweep_acc": selection_method.sweep_acc(group["records"]) }
        ).filter(lambda g: g["sweep_acc"] is not None)
    # print('grouped_records', grouped_records)
    ...
    for dataset in dataset_names:
        ...
        to_save = {algorithm_name: [] for algorithm_name in alg_names}
        for i, algorithm in enumerate(alg_names):
            ...
            to_save[algorithm].extend(means)
        # print('to_save', to_save)
        # np.save('history/' + NAME + '.npy', to_save)
...
if __name__ == "__main__":
    ...
    SELECTION_METHODS = [
        ...
        model_selection.MMDSelectionMethod,
        model_selection.CoralSelectionMethod, 
    ...
    if args.latex:
        print("\\end{document}")
# model_selection.IIDAccuracySelectionMethod, model_selection.IIDAccuracySelectionMethod,
        # model_selection.CoralSelectionMethod,         model_selection.MMDSelectionMethod,
        # model_selection.OracleSelectionMethod model_selection.OracleSelectionMethod
